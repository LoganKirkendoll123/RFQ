...
import { ChevronDown, ChevronUp, MapPin, Package, Clock, Thermometer, XCircle, CheckCircle, Truck, Grid } from 'lucide-react';
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-gray-700">
                {result.quotes.length} service option{result.quotes.length !== 1 ? 's' : ''} from {
                  new Set(result.quotes.map(q => q.carrier.name)).size
                } carrier{new Set(result.quotes.map(q => q.carrier.name)).size !== 1 ? 's' : ''}
              </span>
            </div>
            <div className="flex items-center space-x-3">
              <button
